...
for scratch in scratches:
    ...
    copies[scratch[0]] = list(range(scratch[0] + 1, scratch[0] + len(your_win_nums) + 1))

print(copies)


# @functools.cache
def get_added_cards(num):
    return 1 + sum(get_added_cards(n) for n in copies[num])


cards = 0

for scratch in scratches:
    cards += get_added_cards(scratch[0])
